...
describe('keywords and identifiers', () => {
  test('recognizes keywords', () => {
    ...
    expect(tokens.slice(0, -1).every((t) => t.type === TokenType.KEYWORD)).toBe(
      true
    );
...
describe('literals', () => {
  test('recognizes integer literals', () => {
    ...
    const tokens = lexer.tokenize();
    expect(tokens.slice(0, -1).every((t) => t.type === TokenType.INTEGER)).toBe(
      true
    );
  ...
  test('recognizes string literals', () => {
    ...
    const tokens = lexer.tokenize();
    expect(tokens.slice(0, -1).every((t) => t.type === TokenType.STRING)).toBe(
      true
    );
...
describe('line and column tracking', () => {
  ...
  test('tracks column numbers correctly', () => {
    ...
    const lexer = new CLexer(code);
    const tokens = lexer.tokenize();
    expect(tokens.slice(0, -1).map((t) => t.column)).toEqual([1, 5, 7, 9, 11]);
  });
});
